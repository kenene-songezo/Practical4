...
// Configure your interrupts here use debounce
void play_pause_isr(void){
    //Write your logis here
	long timeinterrupt = millis();

	if (timeinterrupt - lastInterruptTime > 200){
		if (playing==true){
			printf("playing\n");
			playing = false;
		}
	...
	lastInterruptTime = timeinterrupt;
}
...
int setup_gpio(void){
	...
	// pinMode() for buttons and pullUpControl() for either.
